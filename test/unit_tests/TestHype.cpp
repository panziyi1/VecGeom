...
bool TestHype()
{
  ...
  assert(ApproxEqual<Precision>(Dist, ((fRmax - fRmin) / 2)) && ApproxEqual(normal, vx));

  Dist  = b1.DistanceToOut(-pmidx, vmx);
  valid = b1.Normal(-pmidx + Dist * vmx, normal);
  assert(ApproxEqual<Precision>(Dist, ((fRmax - fRmin) / 2)) && ApproxEqual(normal, vmx));

  Dist  = b1.DistanceToOut(pmidy, vy);
  valid = b1.Normal(pmidy + Dist * vy, normal);
  assert(ApproxEqual<Precision>(Dist, ((fRmax - fRmin) / 2)) && ApproxEqual(normal, vy));

  Dist  = b1.DistanceToOut(-pmidy, vmy);
  valid = b1.Normal(-pmidy + Dist * vmy, normal);
  assert(ApproxEqual<Precision>(Dist, ((fRmax - fRmin) / 2)) && ApproxEqual(normal, vmy));
  ...
  assert(ApproxEqual<Precision>(Dist, distZ)); //&& ApproxEqual(normal,vz));

  Dist = b1.DistanceToOut(pmidx, vmz);
  assert(ApproxEqual<Precision>(Dist, distZ)); //&& ApproxEqual(normal,vmz));
  ...
  assert(ApproxEqual<Precision>(Dist, 100 - fRmax));
  ...
  assert(ApproxEqual<Precision>(Dist3, Dist4));
