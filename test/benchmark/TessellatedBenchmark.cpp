...
void EmbreeMeshShape::InitMeshFromTesselatedSolid(const UnplacedTessellated& tsl) {
  ...
  Vertex *vertices =
      (Vertex *)rtcSetNewGeometryBuffer(geom_0, RTC_BUFFER_TYPE_VERTEX, 0, RTC_FORMAT_FLOAT3, 4 * sizeof(float), nvertices);
  Triangle *triangles =
      (Triangle *)rtcSetNewGeometryBuffer(geom_0, RTC_BUFFER_TYPE_INDEX, 0, RTC_FORMAT_UINT3, 3 * sizeof(int), ntriangles);
...
int main(int argc, char *argv[])
{
  ...
  auto nfacets2 = tsl.GetNFacets();
  assert(nfacets == nfacets2);
