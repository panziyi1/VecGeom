...
static VToyNavigator const* GetNavigator(LogicalVolume const*lvol){
  return (VToyNavigator const*)lvol->GetUserExtensionPtr();
}
...
public:
  ...
  VToyNavigator const *specialnav_;      // Special navigator for this basket(volume)
...
int ScalarNavigation(Vector3D<Precision> &p, Vector3D<Precision> const &dir, NavigationState *&curnavstate, NavigationState *&newnavstate) {
  ...
  while( ! curnavstate->IsOutside() ) {
    ...
    VToyNavigator const *specialnav = GetNavigator(curnavstate->Top()->GetLogicalVolume());
...
// we assume that that curnavstates are already initialized correctly
  ...
  while(!(*curnavstates)[0]->IsOutside()) {
     ...
     // std::cout << "tracking in " << (*curnavstates)[0]->Top()->GetLogicalVolume()->GetName() << "\n";
        // get navigator object and move point
      VToyNavigator const *specialnav = GetNavigator((*curnavstates)[0]->Top()->GetLogicalVolume());
...
void TestVectorNavigation() {
  ...
  double *steps    = (double*) _mm_malloc(np*sizeof(double),ALIGN_PADDING);
  double *psteps    = (double*) _mm_malloc(np*sizeof(double),ALIGN_PADDING);
  SimpleNavigator nav;
