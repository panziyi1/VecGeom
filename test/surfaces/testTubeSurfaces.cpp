...
void CreateTwoNestedTubes(double, double,
                          double, double, double, double, double,
                          double, double, double, double, double);
void CreateIdenticalTubes(double, double,
                          double, double, double, double, double);
void CreateConcatenatedTubes(double, double,
                             double, double, double, double, double);
void CreateLayeredGeometry(double, double,
...
bool ShootOneParticle(double, double,
...
int main(int argc, char *argv[]){
  ...
  case 0:
    ...
    CreateLayeredGeometry(worldRadius, worldZ,
                          layers, absorbers, rmin, sphi, dphi);
    break;
  case 1:
  ...
  case 2:
    ...
    CreateTwoNestedTubes(worldRadius, worldZ,
                         1, worldRadius/2, worldZ/2, 0, vecgeom::kTwoPi,
                         1, worldRadius/3, worldZ/3, sphi, dphi);
    break;
  case 3:
    std::cout << "Creating identical tubes..." << std::endl;
    CreateIdenticalTubes(worldRadius, worldZ,
                         worldRadius/3, worldRadius/2, worldZ/2, 1, 3);
    break;
  case 4:
...
void CreateSimpleTube(double worldR, double worldZ, double rmin, double rmax, double z, double sphi, double dphi){
  ...
  auto const tubeSolid = vecgeom::GeoManager::MakeInstance<vecgeom::UnplacedTube>(
...
void CreateTwoNestedTubes(double worldR, double worldZ,
                          double rmin,  double rmax,  double z,  double sphi,  double dphi,
  ...
  auto const outerTube = vecgeom::GeoManager::MakeInstance<vecgeom::UnplacedTube>(
    rmin, rmax, z, sphi, dphi);
  auto outerTubeLogic = new vecgeom::LogicalVolume("OuterTube", outerTube);
  worldLogic->PlaceDaughter(outerTubeLogic, &origin);

  auto const innerTube = vecgeom::GeoManager::MakeInstance<vecgeom::UnplacedTube>(
    rmin2, rmax2, z2, sphi2, dphi2);
...
void CreateIdenticalTubes(double worldR, double worldZ,
                          double rmin, double rmax, double z, double sphi, double dphi)
{
  auto worldSolid = new vecgeom::UnplacedBox(worldR, worldR, worldZ);
  auto worldLogic = new vecgeom::LogicalVolume("World", worldSolid);
...
void CreateConcatenatedTubes(double worldR, double worldZ,
                             double rmin, double rmax, double z, double sphi, double dphi)
{
  auto worldSolid = new vecgeom::UnplacedBox(worldR, worldR, worldZ);
  auto worldLogic = new vecgeom::LogicalVolume("World", worldSolid);
  ...
  auto const lowerTube = vecgeom::GeoManager::MakeInstance<vecgeom::UnplacedTube>(
    rmin, rmax, z, sphi, dphi);
  ...
  auto const upperTube = vecgeom::GeoManager::MakeInstance<vecgeom::UnplacedTube>(
    rmin, rmax, z, sphi, dphi);
...
void CreateLayeredGeometry(double worldRadius, double worldZ,
                          int NbOfLayers, int NbOfAbsorbers, double rmin, double sphi, double dphi)
{
  ...
  // Calorimeter
  //
  auto calorSolid = vecgeom::GeoManager::MakeInstance<vecgeom::UnplacedTube>(
                    0, CalorSizeR, 0.5 * CalorThickness, 0, vecgeom::kTwoPi);
  ...
  // Layers
  //
  auto layerSolid = vecgeom::GeoManager::MakeInstance<vecgeom::UnplacedTube>(
                    rmin, CalorSizeR, 0.5 * LayerThickness, sphi, dphi);
  
  //
  // Absorbers
  //
  auto gapSolid = vecgeom::GeoManager::MakeInstance<vecgeom::UnplacedTube>(
                    rmin, CalorSizeR, 0.5 * GapThickness, sphi, dphi);
  ...
  auto absorberSolid = vecgeom::GeoManager::MakeInstance<vecgeom::UnplacedTube>(
                    rmin, CalorSizeR, 0.5 * AbsorberThickness, sphi, dphi);
...
bool ValidateNavigation(int npoints, int nbLayers, vgbrep::SurfData<vecgeom::Precision> const &surfdata,
                        double worldR, double worldZ, double scale)
{
...
bool ShootOneParticle(double worldR, double worldZ,
                      ...
                      vgbrep::SurfData<vecgeom::Precision> const &surfdata)
{
  ...
  if (out_state.GetNavIndex() != outputStates[0]->GetNavIndex() || std::abs(distance - refSteps[0]) > tolerance){
    ...
    std::cout << " IN VOLUME MODEL: (navind) " << outputStates[0]->GetNavIndex() << "; (distance) " << refSteps[0] << std::endl;
...
void TestPerformance(double worldRadius, int npoints, int nbLayers, vgbrep::SurfData<vecgeom::Precision> const &surfdata)
{
  const double CalorSizeR = worldRadius;
  const double GapThickness      = 2.3;
  const double AbsorberThickness = 5.7;
