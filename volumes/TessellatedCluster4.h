...
public:
  ...
  Vector3D<Real_v> fSideVectors[4]; ///< Side vectors of the triangular facets
  Vector3D<Real_v> fVertices[4];    ///< Vertices stored in SIMD format
  ...
  T ComputeSparsity(int &nblobs, int &nfacets)
  {
    ...
    for (unsigned ifacet = 0; ifacet < kVecSize; ++ifacet) {
      T facetsizesq = 0;
      for (int i = 0; i < 4; ++i) {
        lensq = (fFacets[ifacet]->fVertices[i] - fFacets[ifacet]->fVertices[(i + 1) % 4]).Mag2();
  ...
  /** @brief Fill the components 'i' of the cluster with facet data
    * @param index Quadrilateral index, equivalent to SIMD lane index
    * @param facet Quadrilateral facet data
    */
  VECCORE_ATT_HOST_DEVICE
  void AddFacet(size_t index, Facet_t *facet, size_t ifacet)
  {
    ...
    for (size_t ivert = 0; ivert < 4; ++ivert) {
      ...
      Vector3D<T> sideVector                    = facet->fSideVectors[ivert];
  ...
  T SafetySq(Vector3D<T> const &point, int &isurf) const
  {
    ...
    Real_v[4] safetyv_outbound;
    for (int ivert = 0; ivert < 4; ++ivert) {
      safetyv_outbound[ivert] =
          DistanceToLineSegmentSquared2(fVertices[ivert], fVertices[(ivert + 1) % 4], pointv, !withinBound);
    }
    Real_v safety_outv = vecCore::math::Min(safetyv_outbound[0], safetyv_outbound[1]);
    safety_outv = vecCore::math::Min(safetyv_outbound[2], safetyv_outbound[3], safety_outv);
