...
public:
  ...
  //!   - if ray leaves volume: out_state.Top() will point to mother
  //!   - if step limit > step: out_state == in_state
  //!
  //! Otherwise, no full relocation step is done
  virtual Precision ComputeStep(Vector3D<Precision> const & /*globalpoint*/,
                                Vector3D<Precision> const & /*globaldir*/,
                                Precision /*(physics) step limit */,
                                NavigationState const & /*in_state*/,
                                NavigationState & /*out_state*/) const = 0;
