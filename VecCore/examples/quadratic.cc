                   ...
                   size_t N, const char* name)
{
  ...
  for (size_t i = 0; i < N; i += VectorSize<Float_v>())
    QuadSolveSIMD<Backend>(
      (Float_v &)(a[i]),  (Float_v &)(b[i]),  (Float_v &)(c[i]),
...
int main(int argc, char *argv[]) {
  ...
  TestQuadSolve<backend::Scalar>(a, b, c, x1, x2, roots, N, "simd code via plain scalar backend");
  TestQuadSolve<backend::ScalarWrapper>(a, b, c, x1, x2, roots, N, "simd code via scalarwrapper backend");
  TestQuadSolve<backend::SIMD<8>>(a, b, c, x1, x2, roots, N, "simd code via portable simd::array<8> backend");
  TestQuadSolve<backend::SIMD<16>>(a, b, c, x1, x2, roots, N, "simd code via portable simd::array<16> backend");
  TestQuadSolve<backend::SIMD<24>>(a, b, c, x1, x2, roots, N, "simd code via portable simd::array<24> backend");
  TestQuadSolve<backend::SIMD<32>>(a, b, c, x1, x2, roots, N, "simd code via portable simd::array<32> backend");
