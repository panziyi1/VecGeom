...
bool GenerateNavigatorLibrary(std::vector<std::string> const &volumenames)
{
  if (volumenames.size() == 0) {
    std::cerr << "GenerateNavigatorLibrary: at least one volume name is needed\n";
    return false;
  }

  std::vector<std::string> navigatornames;

  for (const auto &vname : volumenames) {
    navigatornames.push_back(std::string(vname) + std::string("Navigator"));
  }
