...
void GenerateCMakeFile(std::ostream &ss) {
  ...
  ss << "# actually this should come from VecGeom if needed (because it seems that the LIBS are also coming "
        "automatically):\n";
  ss << "find_package(Vc REQUIRED)\n";
  ss << "include_directories(${Vc_INCLUDE_DIR})\n";

  ss << "find_package(ROOT REQUIRED)\n";
  ss << "include_directories(AFTER SYSTEM ${ROOT_INCLUDE_DIRS})\n";

  ss << "message(STATUS \"COMPILING WITH ${CMAKE_CXX_FLAGS}\")\n";

  ss << "add_library(GeneratedNavigators SHARED navigatorlib.cpp)\n";
  ss << "target_link_libraries(GeneratedNavigators ${VECGEOM_LIBRARIES} ${Vc_LIBRARIES})\n";
}
