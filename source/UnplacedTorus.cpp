...
{
  printf("UnplacedTorus {%.2f, %.2f, %.2f, %.2f, %.2f}", rmin(), rmax(), rtor(), sphi(), dphi());
}

void UnplacedTorus::Print(std::ostream &os) const
{
  os << "UnplacedTorus {" << rmin() << ", " << rmax() << ", " << rtor() << ", " << sphi() << ", " << dphi();
}
...
{
  if (placement) {
    new (placement) SpecializedTorus<transCodeT, rotCodeT>(logical_volume, transformation
#ifdef VECCORE_CUDA
                                                           ,
                                                           NULL, id
#endif
  ...
  return new SpecializedTorus<transCodeT, rotCodeT>(logical_volume, transformation
#ifdef VECCORE_CUDA
                                                    ,
                                                    NULL, id
#endif
...
VECCORE_ATT_DEVICE
VPlacedVolume *UnplacedTorus::SpecializedVolume(LogicalVolume const *const volume,
                                                Transformation3D const *const transformation,
                                                ...
                                                VPlacedVolume *const placement) const
{
  ...
  return VolumeFactory::CreateByTransformation<UnplacedTorus>(volume, transformation, trans_code, rot_code,
#ifdef VECCORE_CUDA
                                                              id,
#endif
                                                              placement);
...
Vector3D<Precision> UnplacedTorus::GetPointOnSurface() const
{
  ...
  if ((fSphi == 0.) && (fDphi == vecgeom::kTwoPi)) {
    aSide = 0;
  }
  chose = RNG::Instance().uniform(0., aOut + aIn + 2. * aSide);

  if (chose < aOut) {
    return Vector3D<Precision>((fRtor + fRmax * cosv) * cosu, (fRtor + fRmax * cosv) * sinu, fRmax * sinv);
  } else if ((chose >= aOut) && (chose < aOut + aIn)) {
    return Vector3D<Precision>((fRtor + fRmin * cosv) * cosu, (fRtor + fRmin * cosv) * sinu, fRmin * sinv);
  } else if ((chose >= aOut + aIn) && (chose < aOut + aIn + aSide)) {
    rRand = volumeUtilities::GetRadiusInRing(fRmin, fRmax);
    return Vector3D<Precision>((fRtor + rRand * cosv) * std::cos(fSphi), (fRtor + rRand * cosv) * std::sin(fSphi),
                               rRand * sinv);
  } else {
    rRand = volumeUtilities::GetRadiusInRing(fRmin, fRmax);
...
std::ostream &UnplacedTorus::StreamInfo(std::ostream &os) const
{
...
VECCORE_ATT_HOST_DEVICE
void UnplacedTorus::DetectConvexity()
{
...
DevicePtr<cuda::VUnplacedVolume> UnplacedTorus::CopyToGpu(DevicePtr<cuda::VUnplacedVolume> const in_gpu_ptr) const
{
...
bool UnplacedTorus::Normal(Vector3D<Precision> const &point, Vector3D<Precision> &norm) const
{
  ...
  //
  const Precision delta  = Max(10.0 * kTolerance, 1.0e-8 * (fRtor + fRmax));
...
template void DevicePtr<cuda::UnplacedTorus>::Construct(const Precision rmin, const Precision rmax,
