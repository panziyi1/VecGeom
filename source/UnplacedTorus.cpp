...
Vector3D<Precision> UnplacedTorus::SamplePointOnSurface() const
{
  // taken from Geant4
  ...
  if (chose < aOut) {
    return Vector3D<Precision>((fTorus.fRtor + fTorus.fRmax * cosv) * cosu, (fTorus.fRtor + fTorus.fRmax * cosv) * sinu, fTorus.fRmax * sinv);
  } else if ((chose >= aOut) && (chose < aOut + aIn)) {
    return Vector3D<Precision>((fTorus.fRtor + fTorus.fRmin * cosv) * cosu, (fTorus.fRtor + fTorus.fRmin * cosv) * sinu, fTorus.fRmin * sinv);
...
void UnplacedTorus::DetectConvexity()
{
  ...
  if (fTorus.fRtor == 0.) {   // This will turn Torus to Spherical Shell
...
{
  return CopyToGpuImpl<UnplacedTorus>(in_gpu_ptr, fTorus.fRmin, fTorus.fRmax, fTorus.fRtor, fTorus.fSphi, fTorus.fDphi);
}
