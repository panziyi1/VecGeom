...
void NavStateIndex::TopMatrixImpl(NavIndex_t nav_ind, Transformation3D &trans)
{
  ...
  if (has_matrix) {
    ...
    TopMatrixImpl(NavInd(nav_ind), trans);
    trans.MultiplyFromRight(*TopImpl(nav_ind)->GetTransformation());
  }
}
...
{
  if (IsOutside()) {
    printf("NavState: Outside setup\n");
    return;
  }
  auto level = GetLevel();
  printf("NavState: navInd=%u, level=%u,  onBoundary=%s, path=<", fNavInd, level, (fOnBoundary ? "true" : "false"));
  for (int i = 0; i < level + 1; ++i) {
    auto nav_ind = GetNavIndexImpl(fNavInd, level);
    printf("/%u", nav_ind);
  }
  printf(">\n");
}
...
NavStateIndex &NavStateIndex::operator=(TGeoBranchArray const &other)
{
  ...
  for (auto i = 0; i < other.GetLevel(); ++i) {
